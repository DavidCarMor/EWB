function [data,opt,datags]=EPI_WBV9(data,opt)
%% Function details
...
%   - Se_max: Maximum water level of epiphyte tank in mm,
...
arguments % Defaults
    ...
    opt.Te_Ta double = -0.5 % Initial guess for difference between Te and Ta [K]
...
data.e_a = zeros(size(data,1),1); % Water vapor pressure of air [Pa]
data.e_epi = zeros(size(data,1),1); % Water vapor pressure of the epiphyte mat [Pa],
data.em_a = zeros(size(data,1),1); % Air emissivity
data.es_a = zeros(size(data,1),1); % Water vapor pressure of saturated air  [Pa]
data.es_epi = zeros(size(data,1),1); % Water vapor pressure of saturated epiphyte mat [Pa]
data.ETe = zeros(size(data,1),1); % Evapotranspiration from epiphyte mats  [mm h-1]
data.ETht = zeros(size(data,1),1); % Evapotranspiration from host tree [mm h-1]
data.g_ba = zeros(size(data,1),1); % Canopy boundary later  [m s-1]
data.g_e = zeros(size(data,1),1); % Epiphyte mat water conductance  [m s-1]
data.g_sve = zeros(size(data,1),1); % Stomatal conductance of vascular epiphytes [m s-1]
data.H = zeros(size(data,1),1); % Sensible heat [W m-2]
data.L = zeros(size(data,1),1); % Latent heat [W m-2]
data.Ie = zeros(size(data,1),1); % Interception of rainfall and fog [mm]
data.phi_net = zeros(size(data,1),1); % Net radiation received by the epiphyte mat  [W m-2]
data.psi_e = zeros(size(data,1),1); % Epiphyte mat water potential [m]
data.Se = zeros(size(data,1),1); % Epiphyte mat water content [mm h-1]
data.SF = zeros(size(data,1),1); % Stemflow [mm h-1]
data.sm = zeros(size(data,1),1); % Epiphyte mat moisture [kg kg-1]
data.Te_C = zeros(size(data,1),1); % Epiphyte mat temperature [C]
data.Te_K = zeros(size(data,1),1); % Epiphyte mat temperature [K]
data.TF = zeros(size(data,1),1); % Throughfall[mm h-1]
data.theta_e = zeros(size(data,1),1); % Epiphyte mat volumectric water content [m3/m3]
data.VPD = zeros(size(data,1),1); % Air vapor pressure deficit [Pa]
data.WUht = zeros(size(data,1),1); % Host tree water uptake [mm h-1]
...
% Maximum water level of epiphyte tank [mm]
...
%% Initial variables in the epiphyte tank
...
data.Te_C(1) = data.Ta_C(1)+opt.Te_Ta; % [C]
data.Te_K(1) = data.Ta_K(1)+opt.Te_Ta; % [K]
...
%% Count variable to stabilize the model
if data.Se(1) > 12 && rem(length(data.Se),24)==0
    [data_s,mcount]=spinUp([data(1:24,:);data(1:24,:)],opt,0,0);
    opt.mcount = mcount; % Number of times we used the first day to run 
    % the model until it stabilized.
    data.Te_K(1) = data_s.Te_K(1); % Replace initial Te_K
    data.Te_C(1) = data_s.Te_C(1); % Replace initial Te_C
    % data.Se(1) = data_s.Se(1); % Replace initial Se
    % data.theta_e(1) = data_s.theta_e(1); % % Replace initial theta_e
else
    warning('The simulation should have at least 24 hours of data')
    return
end
...
for i = 1:size(data,1)
    ...
    % Weighted average for epiphyte tank conductance
    ...
    if i < size(data,1) && abs(data.Te_K(i)-data.Te_K(i+1)) >= 10 && i > 1
        ...
        data(i+1,:) = [];
        warning(['The simulation stopped because the epiphyte tank temperature ' ...
...
function cos_out = cos_zenith(data,latitude,t0_param,day_l)
    ...
    for i= 1:size(uniq_dt,1)
        % t0(i,1) = find(data.SW_Wpms(dt==uniq_dt(i,1))>0, 1, 'first')-1;
        % delta(i,1) = find(data.SW_Wpms(dt==uniq_dt(i,1))>0, 1, 'last')-1;
        % delta(i,1) = delta(i,1) - t0(i,1);
        sn(i,1) =find(data.SW_Wpms(dt==uniq_dt(i,1)) == ...
            max(data.SW_Wpms(dt==uniq_dt(i,1)) ), 1, 'first')-1;
    end
    % t0 = repelem(t0,24,1);
    % delta = repelem(delta,24,1);
    sn = repelem(sn,24,1);
    gamma_DOY = (2*pi/365)*((day(data.Date,'dayofyear')-1)+((hour(data.Date)-sn)/24));
    ...
    % Hour angle
    h_r = 15.*((hour(data.Date)+(Offset./60))-sn);
...
%% Spinning Up initial temperature
function [data,mcount] = spinUp(data,opt,mcount,tr)
% Adding a row for keeping spninning with a forecast
data = [data; data(1,:)];

% Desription

% data = first day data to stabilize +1
% opt = model parameters
% mcount = iteration count
% tr = Temperature trend
...
for i = 1:size(data,1)
    ...
    % regulated without that variable conductance
    if i < size(data,1) && abs(data.Te_K(i)-data.Te_K(i+1)) >= 10 && i > 1
        data.Te_K(i+1) = data.Te_K(i);
    end

    % Stopping model when abs(data.Tepi_K(i)-data.Tepi_K(i-1)) >= 10
    if i < size(data,1) && data.theta_e(i+1) <= 0  && i > 1
        data.theta_e(i+1) = data.theta_e(i);
    end
end
% RECURSIVE FUNCTION UNTIL WE MEET A GOOD STABILITY
% Based on MATLAB's double-precision and average ETe in a day
if abs(data.Te_C(end)-data.Te_C(1)) ~= tr && mcount < 1000
    tr = abs(data.Te_C(end)-data.Te_C(1));
    % Updating temperature first guess
    data.Te_C(1) = data.Te_C(end);
    data.Te_C(2:size(data,1),1) = zeros(size(data,1)-1,1); % Epiphyte temperature [C]
    
    % Restarting tank temperature
    data.Te_K(1) = data.Te_K(end);
    data.Te_K(2:size(data,1),1) = zeros(size(data,1)-1,1); % Epiphyte temperature [K]

    % Restarting water level data
    data.Se(2:size(data,1),1) = zeros(size(data,1)-1,1); % Tank level[C]
    
     % Restarting volumetric water content
    data.theta_e(2:size(data,1),1) = zeros(size(data,1)-1,1); % Tank level[C]
    
    % Removing extra line because of for loop
    data = data(1:end-1,:);
    [data,mcount] = spinUp(data,opt,mcount,tr);
end
